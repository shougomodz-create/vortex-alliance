...
const TURNSTILE_SITEKEY = process.env.TURNSTILE_SITEKEY || '0x4AAAAAACzvNarbs8yzOoKh';
...
router.get('/login', (req, res) => {
  res.send(`<!DOCTYPE html><html lang="pt-BR"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width, initial-scale=1.0"><title>Login - VORTEX</title><link href="https://fonts.googleapis.com/css2?family=Cinzel:wght@400;600;700&family=Inter:wght@400;500&display=swap" rel="stylesheet"><link rel="stylesheet" href="/css/admin.css"><script src="https://challenges.cloudflare.com/turnstile/v0/api.js" async defer></script></head><body class="login-page"><div class="login-container"><div class="login-card"><div class="login-header"><span class="login-icon">⚔</span><h1>ALIANÇA VORTEX</h1><p>Painel Administrativo</p></div><form id="loginForm" class="login-form"><div class="form-group"><label for="username">Usuário</label><input type="text" id="username" name="username" required placeholder="Digite seu usuário"></div><div class="form-group"><label for="password">Senha</label><input type="password" id="password" name="password" required placeholder="Digite sua senha"></div><div class="form-group"><div class="cf-turnstile" data-sitekey="${TURNSTILE_SITEKEY}" data-theme="dark" data-callback="onTurnstileOK" data-error-callback="onTurnstileErr"></div></div><div id="loginError" class="error-message" style="display:none;"></div><button type="submit" class="btn btn-primary btn-full" id="loginBtn" disabled>ENTRAR</button></form><a href="/" class="back-link">← Voltar ao site</a></div></div><script>var _tt=null;function onTurnstileOK(t){_tt=t;document.getElementById("loginBtn").disabled=false}function onTurnstileErr(){_tt=null;document.getElementById("loginBtn").disabled=false}setTimeout(function(){document.getElementById("loginBtn").disabled=false},5000);document.getElementById("loginForm").addEventListener("submit",function(e){e.preventDefault();var er=document.getElementById("loginError");er.style.display="none";var btn=document.getElementById("loginBtn");btn.disabled=true;btn.textContent="ENTRANDO...";fetch("/admin/login",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:document.getElementById("username").value,password:document.getElementById("password").value,turnstileToken:_tt})}).then(function(r){return r.json()}).then(function(d){if(d.success)window.location.replace("/admin");else{er.textContent=d.error||"Erro";er.style.display="block";btn.disabled=false;btn.textContent="ENTRAR";if(typeof turnstile!=="undefined")turnstile.reset()}}).catch(function(){er.textContent="Erro de conexão";er.style.display="block";btn.disabled=false;btn.textContent="ENTRAR"})});</script></body></html>`);
});
...
router.post('/login', async (req, res) => {
  try {
    ...
    if (TURNSTILE_SECRET) {
      if (!turnstileToken) {
        return res.status(400).json({ error: 'Verificação de segurança necessária.' });
      }
...
router.get('/', authMiddleware, (req, res) => {
  const s = {
    o: database.get('SELECT COUNT(*) as c FROM oficiais')?.c||0,
    p: database.get('SELECT COUNT(*) as c FROM parcerias')?.c||0,
    a: database.get('SELECT COUNT(*) as c FROM afiliados')?.c||0,
    vt: database.get('SELECT COUNT(*) as c FROM visits')?.c||0,
    vh: database.get("SELECT COUNT(*) as c FROM visits WHERE date(created_at)=date('now', 'localtime')")?.c||0
  };
  const visits = database.all('SELECT * FROM visits ORDER BY created_at DESC LIMIT 10');
  res.send(layout('Dashboard', '<div class="stats-grid"><div class="stat-card"><div class="stat-icon">👤</div><div class="stat-info"><span class="stat-number">'+s.o+'</span><span class="stat-label">Oficiais</span></div></div><div class="stat-card"><div class="stat-icon">🤝</div><div class="stat-info"><span class="stat-number">'+s.p+'</span><span class="stat-label">Parcerias</span></div></div><div class="stat-card"><div class="stat-icon">👥</div><div class="stat-info"><span class="stat-number">'+s.a+'</span><span class="stat-label">Afiliados</span></div></div><div class="stat-card"><div class="stat-icon">👁</div><div class="stat-info"><span class="stat-number">'+s.vt+'</span><span class="stat-label">Visitas</span></div></div></div><div class="card"><h3>Visitas Recentes</h3><table class="data-table"><thead><tr><th>IP</th><th>Data</th></tr></thead><tbody>'+visits.map(function(v){return '<tr><td>'+(v.ip||'N/A')+'</td><td>'+new Date(v.created_at).toLocaleString('pt-BR')+'</td></tr>'}).join('')+'</tbody></table></div>'));
});

// ===== OFICIAIS =====
router.get('/oficiais', authMiddleware, (req, res) => {
  const data = database.all('SELECT * FROM oficiais ORDER BY ordem ASC');
...
// ===== API CRUD =====
router.post('/api/oficiais', authMiddleware, upload.single('avatarFile'), (req, res) => {
  try {
    ...
    const r = database.run('INSERT INTO oficiais (nome,cargo,descricao,avatar,discord,whatsapp,instagram,ordem,ativo) VALUES (?,?,?,?,?,?,?,?,?)', [nome, cargo, descricao, url, discord, whatsapp, instagram, parseInt(ordem)||0, ativo==='true'?1:0]);
    res.json({ success: true, id: r.lastInsertRowid });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
router.put('/api/oficiais/:id', authMiddleware, upload.single('avatarFile'), (req, res) => {
  try {
    const { nome, cargo, descricao, discord, whatsapp, instagram, ordem, ativo, avatar } = req.body;
    const url = req.file ? '/images/' + req.file.filename : avatar;
    database.run('UPDATE oficiais SET nome=?,cargo=?,descricao=?,avatar=?,discord=?,whatsapp=?,instagram=?,ordem=?,ativo=? WHERE id=?', [nome, cargo, descricao, url, discord, whatsapp, instagram, parseInt(ordem)||0, ativo==='true'?1:0, req.params.id]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
router.delete('/api/oficiais/:id', authMiddleware, (req, res) => { database.run('DELETE FROM oficiais WHERE id=?', [req.params.id]); res.json({ success: true }); });
router.post('/api/oficiais/reorder', authMiddleware, (req, res) => { req.body.items.forEach(i => database.run('UPDATE oficiais SET ordem=? WHERE id=?', [i.ordem, i.id])); res.json({ success: true }); });

router.post('/api/parcerias', authMiddleware, upload.single('logoFile'), (req, res) => {
  try {
    const { nome, tipo, descricao, link, whatsapp, membros, plataforma, categoria, ordem, ativo, logo } = req.body;
    const url = req.file ? '/images/' + req.file.filename : logo;
    const r = database.run('INSERT INTO parcerias (nome,tipo,descricao,logo,link,whatsapp,membros,plataforma,categoria,ordem,ativo) VALUES (?,?,?,?,?,?,?,?,?,?,?)', [nome, tipo, descricao, url, link, whatsapp, parseInt(membros)||0, plataforma||'Discord', categoria||'Geral', parseInt(ordem)||0, ativo==='true'?1:0]);
    res.json({ success: true, id: r.lastInsertRowid });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
router.put('/api/parcerias/:id', authMiddleware, upload.single('logoFile'), (req, res) => {
  try {
    const { nome, tipo, descricao, link, whatsapp, membros, plataforma, categoria, ordem, ativo, logo } = req.body;
    const url = req.file ? '/images/' + req.file.filename : logo;
    database.run('UPDATE parcerias SET nome=?,tipo=?,descricao=?,logo=?,link=?,whatsapp=?,membros=?,plataforma=?,categoria=?,ordem=?,ativo=? WHERE id=?', [nome, tipo, descricao, url, link, whatsapp, parseInt(membros)||0, plataforma||'Discord', categoria||'Geral', parseInt(ordem)||0, ativo==='true'?1:0, req.params.id]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
router.delete('/api/parcerias/:id', authMiddleware, (req, res) => { database.run('DELETE FROM parcerias WHERE id=?', [req.params.id]); res.json({ success: true }); });
router.post('/api/parcerias/reorder', authMiddleware, (req, res) => { req.body.items.forEach(i => database.run('UPDATE parcerias SET ordem=? WHERE id=?', [i.ordem, i.id])); res.json({ success: true }); });

router.post('/api/afiliados', authMiddleware, upload.single('logoFile'), (req, res) => {
  try {
    const { nome, tipo, descricao, link, whatsapp, membros, plataforma, categoria, ordem, ativo, logo } = req.body;
    const url = req.file ? '/images/' + req.file.filename : logo;
    const r = database.run('INSERT INTO afiliados (nome,tipo,descricao,logo,link,whatsapp,membros,plataforma,categoria,ordem,ativo) VALUES (?,?,?,?,?,?,?,?,?,?,?)', [nome, tipo, descricao, url, link, whatsapp, parseInt(membros)||0, plataforma||'Discord', categoria||'Geral', parseInt(ordem)||0, ativo==='true'?1:0]);
    res.json({ success: true, id: r.lastInsertRowid });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
router.put('/api/afiliados/:id', authMiddleware, upload.single('logoFile'), (req, res) => {
  try {
    const { nome, tipo, descricao, link, whatsapp, membros, plataforma, categoria, ordem, ativo, logo } = req.body;
    const url = req.file ? '/images/' + req.file.filename : logo;
    database.run('UPDATE afiliados SET nome=?,tipo=?,descricao=?,logo=?,link=?,whatsapp=?,membros=?,plataforma=?,categoria=?,ordem=?,ativo=? WHERE id=?', [nome, tipo, descricao, url, link, whatsapp, parseInt(membros)||0, plataforma||'Discord', categoria||'Geral', parseInt(ordem)||0, ativo==='true'?1:0, req.params.id]);
    res.json({ success: true });
  } catch(e) { res.status(500).json({ error: e.message }); }
});
router.delete('/api/afiliados/:id', authMiddleware, (req, res) => { database.run('DELETE FROM afiliados WHERE id=?', [req.params.id]); res.json({ success: true }); });
router.post('/api/afiliados/reorder', authMiddleware, (req, res) => { req.body.items.forEach(i => database.run('UPDATE afiliados SET ordem=? WHERE id=?', [i.ordem, i.id])); res.json({ success: true }); });

router.post('/api/settings', authMiddleware, (req, res) => {
  Object.entries(req.body).forEach(([k, v]) => {
    if (k.startsWith('setting_')) {
      const key = k.replace('setting_', '');
      const ex = database.get('SELECT id FROM settings WHERE key=?', [key]);
      if (ex) database.run('UPDATE settings SET value=? WHERE key=?', [v, key]);
      else database.run('INSERT INTO settings (key,value) VALUES (?,?)', [key, v]);
    }
  });
  res.json({ success: true });
});
